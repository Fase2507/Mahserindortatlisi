...
function HandSynergy({ count }: { count: number }) {
  const hands = [
    { dir: "bottom" as Dir, color: "#0f172a", delay: 0 },
    { dir: "top"    as Dir, color: "#475569", delay: 0.07 },
    { dir: "left"   as Dir, color: "#dc2626", delay: 0.14 },
    { dir: "right"  as Dir, color: "#2563eb", delay: 0.21 },
  ]
  const isV = (d: Dir) => d === "bottom" || d === "top"
  const from = (d: Dir) => ({ bottom:{y:90,opacity:0}, top:{y:-90,opacity:0}, left:{x:-90,opacity:0}, right:{x:90,opacity:0} })[d]
  const to   = (d: Dir) => ({ bottom:{y:22,x:0,opacity:1}, top:{y:-22,x:0,opacity:1}, left:{x:-22,y:0,opacity:1}, right:{x:22,y:0,opacity:1} })[d]
    ...
    <div className="relative w-32 h-32 flex items-center justify-center shrink-0">
      ...
      <AnimatePresence>
        {hands.slice(0, count).map(h => (
          <motion.div key={h.dir}
            initial={from(h.dir)} animate={to(h.dir)} exit={from(h.dir)}
            transition={{ type: "spring", stiffness: 240, damping: 22, delay: h.delay }}
            style={{ position:"absolute", backgroundColor: h.color,
              width: isV(h.dir) ? 22 : 66, height: isV(h.dir) ? 66 : 22,
              borderRadius: 99 }}
          >
            <div style={{ position:"absolute", borderRadius:99, backgroundColor:"rgba(255,255,255,0.25)",
              ...(isV(h.dir) ? {top:6,bottom:6,left:3,width:4} : {left:6,right:6,top:3,height:4}) }} />
          </motion.div>
        ))}
      </AnimatePresence>
      <motion.div key={count} initial={{scale:0,opacity:0.5}} animate={{scale:3,opacity:0}}
        transition={{duration:0.45,ease:"easeOut"}}
        style={{position:"absolute",width:20,height:20,borderRadius:99,backgroundColor:"#cbd5e1"}} />
      <div style={{position:"relative",zIndex:50,width:26,height:26,borderRadius:99,
        backgroundColor:"white",border:"2px solid #e2e8f0",display:"flex",alignItems:"center",justifyContent:"center",
        boxShadow:"0 1px 4px rgba(0,0,0,0.08)"}}>
        <span style={{fontSize:10,fontWeight:900,color:"#475569"}}>{count}</span>
      </div>
    </div>
  )
}
                          ...
                          {[2,3,4].map(n => (
                            <button key={n} type="button" onClick={() => setCount(n)}
                              style={{
                                ...
                                boxShadow: members.length===n ? "0 4px 16px rgba(15,23,42,0.18)" : "none",
